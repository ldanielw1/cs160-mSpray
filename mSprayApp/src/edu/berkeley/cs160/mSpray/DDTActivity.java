...
public class DDTActivity extends BaseMainActivity {
...
//				intent.putExtra(Constants.NUM_SPRAYERS, DataStore.numSprayer);
...
//				startActivity(intent);
	...
	public void getData() {
		...
		if (roomsSprayedValue.getText().toString().equals("")
			...
			Toast.makeText(getApplicationContext(),
					"Please input a value for every text field",
					Toast.LENGTH_SHORT).show();
		} else {
			try {
				int roomsSprayed = Integer.parseInt(roomsSprayedValue.getText()
						.toString());
				int sheltersSprayed = Integer.parseInt(sheltersSprayedValue
						.getText().toString());
				...
				Toast.makeText(getApplicationContext(),
						"Please input integer values in every text field",
						Toast.LENGTH_SHORT).show();
			}
		}
	}
	 ...
	 * first sprayer. This also avoids having a million activities on top of
	 * each other in cases where the sprayer can't make up their mind.
	 */
	@Override
