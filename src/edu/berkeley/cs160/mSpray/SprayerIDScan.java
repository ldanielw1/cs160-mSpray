...
public class SprayerIDScan extends Activity {
    ...
    Boolean foremanRecorded; // This is different from collectedForemanRFIDs.
                             // collectedForemanRFIDs is for telling this
                             // activity to go add sprayers
    ...
    // Handles variable values; variables like scannedName and addSprayer are
    ...
    private void addSprayers() {
        ...
        setRFIDScannerVisibility(View.VISIBLE);
        addSprayer.setVisibility(View.INVISIBLE); // Back button takes care of
                                                  // "rescan" action
        TimeBomb();
    }
    ...
    private void setRFIDScannerVisibility(int vis) {
        ...
        if (vis == View.INVISIBLE) {
            ...
            // Resize button --> small
            MarginLayoutParams marginParams = new MarginLayoutParams(addSprayer.getLayoutParams());
        ...
        Thread thread = new Thread() {
            ...
            public void run() {
                try {
                    synchronized (this) {
                        wait(3000);
                        handler.sendMessage(Message.obtain(handler, Constants.TIME_BOMB_SWITH));
